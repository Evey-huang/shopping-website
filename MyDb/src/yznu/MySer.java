...
public class MySer extends HttpServlet {
	...
	if(status.equals("insert")){
	    ...
	    boolean fl=db.insert(id,text,bookname,price);
	    if(fl==true){
	    ...
	    boolean fl=db.addcart(id,text,bookname,price);
	    if(fl==true){
			 RequestDispatcher dispatcher=request.getRequestDispatcher("/jsp/addcart.jsp");
			 dispatcher.forward(request, response);
	    	return ;
	    }
	}
		   ...
		   String id=request.getParameter("id"); 
		    ...
		    boolean fl=db.updateStudent(id,text,bookname,price);//
		    if(fl==true){
		    	 int currentPage=Integer.parseInt(request.getParameter("currentPage"));
					//更新session中的当前页
					request.getSession().setAttribute("currentPage", currentPage);
